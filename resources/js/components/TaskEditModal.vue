...
// Watch for task changes and populate form
watch(() => props.task, (newTask) => {
  if (newTask) {
    formData.value = {
      title: newTask.title || '',
      description: newTask.description || '',
      start_date: formatDateForInput(newTask.start_date),
      end_date: formatDateForInput(newTask.end_date),
      user_id: String(newTask.user_id) || '',
      status_id: String(newTask.status_id) || '',
    }
  }
}, { immediate: true })
...
onMounted(() => {
  if (props.task) {
    formData.value = {
      ...
      user_id: String(props.task.user_id) || '',
      status_id: String(props.task.status_id) || '',
    }
  }
})
...
const handleSubmit = () => {
  ...
  router.put(`/tasks/${props.task.id}`, formData.value, {
    onSuccess: () => {
      handleClose()
